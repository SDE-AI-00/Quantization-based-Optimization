...
class Ackley:
    ...
    def __init__(self, d, a=20, b=0.2, c=2*np.pi):
        self.d = d
        self.input_domain = np.array([[-1.5, 1.5] for _ in range(d)])
        self.a = a
        self.b = b
        self.c = c
    ...
    def __call__(self, X):
        ...
        #res = - self.a * np.exp(-self.b * np.sqrt(np.mean(np.power(X, 2.0))))
...
class DropWave:
    ...
    latex_formula_global_minimum = r'f(0, 0)=-1 \Roghtarrow f(0, 0)=0 (a=1)'
    continuous = True
    convex = False
    separable = False
    differentiable = True
    ...
    def __init__(self, d, a=1):
        ...
        self.input_domain = np.array([[-1.0, 1.0], [-1.0, 1.0]])
        #self.input_domain = np.array([[-0.25, 0.25], [-0.25, 0.25]])
    ...
    def __call__(self, X):
        x, y = X
        res = self.a - (1 + np.cos(12*np.sqrt(x**2 + y**2)))/ (0.5*(x**2 + y**2) + 2)
        return res
...
class EggHolder:
    name = 'Egg Holder'
    latex_formula = r'f(x, y) = 977 - (y + 47) sin \left ( \sqrt{| y + 0.5y +47 |} \right ) - x sin \left ( \sqrt{| x - (y + 47)|} \right )'
    latex_formula_dimension = r'd=2'
    latex_formula_input_domain = r'x \in [-512, 512], y \in [-512, 512]'
    latex_formula_global_minimum = r'f(512, 404.2319)=-1 f(522.16060416 413.31588386)=0.088999'
    continuous = False
    convex = False
    separable = False
    differentiable = True
    ...
    def __init__(self, d, a=977.0):
        self.a = a
        self.d = d
        #self.input_domain = np.array([[-512, 512], [-512, 512]])
        self.input_domain = np.array([[400, 600], [300, 500]])
    ...
    def get_global_minimum(self, d):
        ...
        X = np.array([522.16060416, 413.31588386])
        return (X, self(X))

    def __call__(self, X):
        x, y = X
        res = self.a -(y + 47) * np.sin(np.sqrt(np.abs(y + x/2 + 47))) - x * np.sin(np.sqrt(np.abs(x - y - 47)))
        return res
...
class XinSheYangN4:
    ...
    latex_formula = r'f(\mathbf{x})= 2.0 + \left(\sum_{i=1}^{d}sin^2(x_i)-exp(-\sum_{i=1}^{d}x_i^2)\right)exp(-\sum_{i=1}^{d}{sin^2\sqrt{|x_i|}})'
    latex_formula_dimension = r'd \in \mathbb{N}_{+}^{*}'
    latex_formula_input_domain = r'x_i \in [-10, 10], \forall i \in \llbracket 1, d\rrbracket'
    ...
    def __call__(self, X):
        d = X.shape[0]
        res = self.a + np.sum(np.sin(X)**2 - np.exp(-np.sum(X)**2)) * np.exp(-np.sum(np.sin(np.sqrt(np.abs(X)))**2))
        return res
...
class Rosenbrock_Modification:
    name = 'Rosenbrock Modification'
    latex_formula = r'f(\mathbf{x}) = 74 + 100(x_2 - x_1^2)^2 + (1 - x_1)^2 - 400 e^{-\frac{(x_1+1)^2 + (x_2 + 1)^2}{0.1}}'
    latex_formula_dimension = r'd = 2'
    latex_formula_input_domain = r'x_i \in [-2, 2], \forall i \in \llbracket 1, 2\rrbracket'
    latex_formula_global_minimum = r'f(-0.9, -0.95)=34.37'
    continuous = True
    convex = False
    separable = False
    differentiable = True
    ...
    @classmethod
    def is_dim_compatible(cls, d):
        assert (d is None) or (isinstance(d, int) and (not (d < 0 or d > 2)), "The dimension d must be None or a positive integer")
        return  (d is None) or (d == 2)

    def __init__(self, d, a=1, b=100, c=74):
        self.d = d
        self.input_domain = np.array([[-1.3, 0.6] for _ in range(d)])
        self.a = a
        self.b = b
        self.c = c
    ...
    def get_global_minimum(self, d):
        #X = np.array([-0.9, -0.95])
        X = np.array([-0.90953881, -0.95052742])
        return (X, self(X))

    def __call__(self, X):
        d   = X.shape[0]
        _e  = -((X[0] + 1)**2 + (X[1] + 1)**2)/0.1
        res = self.c + self.b * (X[1] - X[0]**2)**2 + (1 - X[0])**2 - 4 * self.b * np.exp(_e)
        return res


class Whitley:
    name = 'Whitley'
    latex_formula = r'f(\mathbf{x}) = 1 + \frac{1}{4000} \sum_{i=1}^n x^2_i - \prod_{i=1}^n cos(\frac{x_i}{\sqrt{i}})'
    latex_formula_dimension = r'd = 2'
    latex_formula_input_domain = r'x_i \in [-512, 512], \forall i \in \llbracket 1, d\rrbracket'
    latex_formula_global_minimum = r'f(0, \cdots, 0)=0'
    continuous = True
    convex = False
    separable = False
    differentiable = True
    ...
    def __init__(self, d, a=4000.0):
        self.d = d
        self.input_domain = np.array([[-10.24, 10.24] for _ in range(d)])
        self.a = a
    ...
    def __call__(self, X):
        ...
        res = 1 + (1.0 * part1)/self.a - (1.0 * part2)
        return res
...
if __name__ == '__main__':
    ...
    f_eggholder = EggHolder(_dimension)
    
    _domain     = f_eggholder.input_domain
    _pos, _val  = f_eggholder.get_global_minimum(_dimension)
    
    print("Input Domain  : ", _domain)
    print("Optimal Point : ", _pos) 
    print("Optimal Cost  : ", _val)
