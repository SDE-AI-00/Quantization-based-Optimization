...
#-------------------------------------------------------------
class Q_Analysis:
    def __init__(self, e_th=0.000001):
        self.max_func   = 0
        self.min_func   = 1000000.0
        self.max_f_cnt  = 0
        self.min_f_cnt  = 0
        self.e_th       = e_th
        self.H_fQ       = 0.0

        self.b_minmax_update = False
    def get_max_func(self, _f_value):
        _a = 1/1.0 * (self.max_f_cnt)
        _x = self.max_func
        if _f_value > self.max_func:
            _x = _x + _a * (_f_value - _x)
            self.max_f_cnt = self.max_f_cnt + 1
        else:
            _x = -1
        return _x

    def get_min_func(self, _f_value):
        _a = 1/1.0 * (self.min_f_cnt)
        _x = self.min_func
        if _f_value <= self.min_func:
            _x = _x + _a * (_f_value - _x)
            self.min_f_cnt = self.min_f_cnt + 1
        else:
            _x = -1
        return _x

    def get_eval_minmax_func(self, _f_value, _grad_f):
        self.b_minmax_update = False
        norm_grad       = torch.norm(_grad_f)
        if norm_grad < self.e_th:
            _update_val = self.get_max_func(_f_value = _f_value)
            if _update_val > 0:
                self.max_func = _update_val
                self.b_minmax_update = True
            else:
                _update_val = self.get_min_func(_f_value=_f_value)
                if _update_val > 0:
                    self.min_func = _update_val
                    self.b_minmax_update = True
                else:
                    pass

        return self.b_minmax_update
    def Q_analysis(self, _f_value, _grad_f, _iterindex):
        if _iterindex == 0:
            DBG.dbg("Debug Test")
        else:
            self.get_eval_minmax_func(_f_value, _grad_f)
