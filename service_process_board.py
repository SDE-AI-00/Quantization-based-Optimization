...
class send_notify:
    def __init__(self, recvEmail="jnwseok@etri.re.kr"):
        ...
        self.smtpName   = _class_config_data['smtpName']  
        self.smtpPort   = _class_config_data['smtpPort']  
        self.etri_mail  = _class_config_data['etri_mail']
        self.text       = ''
        self.msg        = ''
        self._real_msg  = []
        self._head_msg  = _class_config_data['head_msg']
        self._testsite_to_get_IPaddr = ("pwnbit.kr", 443)
    ...
    def send_noti_mail(self, _msg):
        ...
        s = smtplib.SMTP_SSL(self.smtpName, self.smtpPort)  
        ...
        try:
            s.login(self._id, self.password)  
        except Exception as e:
            print("log in Fail")
            print(e)
            exit()

        s.sendmail(self.sendEmail, self.recvEmail, self.msg.as_string())  
        s.close()  
        #s.quit()

    def get_IP_Address(self):
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        sock.connect(self._testsite_to_get_IPaddr)
    ...
    def extract_batfile(self, _batfile="work_02.bat"):
        ...
        _ret_msg += "IP address " + self.get_IP_Address() + "  \n"
